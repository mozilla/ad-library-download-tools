...
class GoogleBigQueryDownloadHelper:
	...
	def _init_db(self):
		url = "sqlite:///{:s}".format(os.path.abspath(os.path.join(self.download_folder, Constants.GOOGLE_AD_LIBRARY_DB_FILENAME)))
	...
	def download_advertiser_declared_stats_table(self):
		...
		with self.engine.begin() as conn:
			...
			for page in rows.pages:
				if self.verbose:
					print("[BigQuery] Page #{:d} ({:d} rows)".format(rows.page_number, rows.num_results))
				for row in page:
		...
		with self.engine.begin() as conn:
			...
			for page in rows.pages:
				if self.verbose:
					print("[BigQuery] Page #{:d} ({:d} rows)".format(rows.page_number, rows.num_results))
				for row in page:
					conn.execute(self.db.last_updated_table.insert(), {
