...
from google_utils import GoogleAdLibraryDB
...
CSV_FILENAME_PREFIX = "google-political-ads-"
...
class GoogleAdReportDownloadHelper:
	...
	def _init_db(self):
		url = "sqlite:///{}".format(os.path.abspath(os.path.join(self.download_folder, Constants.GOOGLE_AD_LIBRARY_DB_FILENAME)))
		self.engine = sqlalchemy.create_engine(url, echo = self.echo)
		self.db = GoogleAdLibraryDB(self.engine)
	...
	def _get_unzipped_csv_filename(self, table):
		return os.path.join(self.download_folder, CSV_FOLDER, "{}{}.csv".format(CSV_FILENAME_PREFIX, table.replace("_", "-")))
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.advertiser_stats_table.insert(), {
							"advertiser_id": row[header["advertiser_id"]],
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.advertiser_weekly_spend_table.insert(), {
							"advertiser_id": row[header["advertiser_id"]],
							"advertiser_name": row[header["advertiser_name"]],
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.campaign_targeting_table.insert(), {
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.creative_stats_table.insert(), {
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.geo_spend_table.insert(), {
			...
			with open(filename) as f:
				...
				for row in reader:
					if header is None:
						...
						conn.execute(self.db.top_keywords_history_table.insert(), {
							"election_cycle": row[header["election_cycle"]],
